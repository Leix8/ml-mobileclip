...
# Ensure cuBLAS workspace (also set again if --deterministic)
...
def taf_fuse_single_ref(text_emb: torch.Tensor, ref_img_emb: torch.Tensor, beta: float, gamma: float) -> torch.Tensor:
    """
    TAF fusion for a single reference image.
    fused = norm((1-β)*t + β*norm(i_parallel) + γ*norm(i_perp))
    """
    dot_it = torch.clamp((ref_img_emb * text_emb).sum(), -1.0, 1.0)
    i_parallel = dot_it * text_emb
    i_perp = ref_img_emb - i_parallel
    return F.normalize((1.0 - beta) * text_emb + beta * safe_norm(i_parallel) + gamma * safe_norm(i_perp), dim=-1)


def taf_fuse_multi_refs(text_emb: torch.Tensor, ref_embs: torch.Tensor, beta: float, gamma: float) -> torch.Tensor:
    """
    TAF for multiple reference embeddings [K, D]: per-ref TAF then mean-aggregate.
    """
    fused_list = [taf_fuse_single_ref(text_emb, ref_embs[k], beta, gamma) for k in range(ref_embs.size(0))]
    return F.normalize(torch.stack(fused_list, dim=0).mean(dim=0), dim=-1)


def cos_push_single_ref(text_emb: torch.Tensor, ref_img_emb: torch.Tensor, push_lambda: float) -> torch.Tensor:
    """
    Cos-Push: explicitly push the reference image embedding along text direction.
    i_push = norm(i + λ t)
    """
    return F.normalize(ref_img_emb + push_lambda * text_emb, dim=-1)


def cos_push_multi_refs(text_emb: torch.Tensor, ref_embs: torch.Tensor, push_lambda: float) -> torch.Tensor:
    """
    Cos-Push for multiple refs: push each, then mean-aggregate and normalize.
    """
    pushed = [cos_push_single_ref(text_emb, ref_embs[k], push_lambda) for k in range(ref_embs.size(0))]
...
def main():
    ...
    parser.add_argument("--ref_images_dir", type=str, default=None,
                        help="Optional directory of multiple reference images for multi-ref fusions.")
    ...
    parser.add_argument("--negatives", type=str, default=None,
                        help="Path to one negative image or a directory of negatives for debiasing.")
    ...
    # ---- Cos-Push (single) ----
    push_lambda = float(args.push_lambda)
    push_vec = cos_push_single_ref(text_emb, ref_img_emb, push_lambda)
    sim_push = cosine_sim(frame_embs, push_vec).cpu().numpy()

    # ---- Multi-ref encodings (optional) ----
    sim_multi_taf = sim_multi_push = None
    sim_multi_taf_debiased = sim_multi_push_debiased = None
    multi_ref_count = 0
    ref_embs_multi = None
    ...
    if args.ref_images_dir:
        ...
        if ref_dir.exists() and ref_dir.is_dir():
            ...
            if multi_ref_paths:
                ref_embs_multi = encode_images(model, image_processor, multi_ref_paths,
                                               device, args.batch_size, use_amp=use_amp)
                ...
                # Multi Cos-Push
                push_multi_vec = cos_push_multi_refs(text_emb, ref_embs_multi, push_lambda)
                sim_multi_push = cosine_sim(frame_embs, push_multi_vec).cpu().numpy()

                # ---- Debiasing if negatives are provided ----
                if args.negatives:
                    neg_path = Path(args.negatives)
                    if neg_path.exists():
                        if neg_path.is_file():
                            neg_embs = encode_images(model, image_processor,
                                                     [neg_path], device, 1, use_amp=use_amp)
                        else:
                            neg_embs = encode_images(model, image_processor,
                                                     list_image_paths(neg_path), device, args.batch_size, use_amp=use_amp)
                        if neg_embs is not None and neg_embs.size(0) > 0:
                            pos_centroid = ref_embs_multi.mean(dim=0)
                            neg_centroid = neg_embs.mean(dim=0)
                            debiased_vec = F.normalize(pos_centroid - neg_centroid, dim=-1)
                            debiased_vec_pushed = F.normalize(pos_centroid - cos_push_single_ref(text_emb, neg_centroid, -0.5), dim=-1)
                            debiased_vec_taf = F.normalize(pos_centroid - taf_fuse_single_ref(text_emb, neg_centroid, -0.4, 0.1), dim=-1)
                            # Debiased TAF
                            taf_multi_debiased = taf_fuse_single_ref(text_emb, debiased_vec_taf, beta, gamma)
                            sim_multi_taf_debiased = cosine_sim(frame_embs, taf_multi_debiased).cpu().numpy()
                            # Debiased Cos-Push
                            push_multi_debiased = cos_push_single_ref(text_emb, debiased_vec_pushed, push_lambda)
                            sim_multi_push_debiased = cosine_sim(frame_embs, push_multi_debiased).cpu().numpy()
    ...
    out_cols = {
        ...
        f"sim_push_lambda_{push_lambda:.2f}": sim_push,
    }
    if sim_multi_taf is not None:
        out_cols[f"sim_multi_taf_beta_{beta:.2f}_gamma_{gamma:.2f}_K_{multi_ref_count}"] = sim_multi_taf
    if sim_multi_push is not None:
        out_cols[f"sim_multi_push_lambda_{push_lambda:.2f}_K_{multi_ref_count}"] = sim_multi_push
    if sim_multi_taf_debiased is not None:
        out_cols[f"sim_multi_taf_debiased_beta_{beta:.2f}_gamma_{gamma:.2f}"] = sim_multi_taf_debiased
    if sim_multi_push_debiased is not None:
        out_cols[f"sim_multi_push_debiased_lambda_{push_lambda:.2f}"] = sim_multi_push_debiased
    ...
    num_rows = 7
    num_rows += (1 if sim_multi_taf is not None else 0) 
    num_rows += (1 if sim_multi_push is not None else 0)
    num_rows += (1 if sim_multi_push_debiased is not None else 0)
    num_rows += (1 if sim_multi_taf_debiased is not None else 0)
    ...
    def plot_curve(ax, title, y, xlabel=False):
        ...
        margin = 0.1 * span
        ax.set_ylim(max(-1.0, ymin - margin), min(1.0, ymax + margin))
    ...
    if sim_multi_taf_debiased is not None:
        ...
        plot_curve(ax8, f"Debiased Multi-Ref TAF", sim_multi_taf_debiased)

    ax9 = fig.add_subplot(gs[row_i]); row_i += 1
    plot_curve(ax9, f"Cos-Push→Frame Similarity (λ={push_lambda:.2f})", sim_push)

    if sim_multi_push is not None:
        ax10 = fig.add_subplot(gs[row_i]); row_i += 1
        plot_curve(ax10, f"Multi-Ref Cos-Push→Frame Similarity (λ={push_lambda:.2f}, K={multi_ref_count})", sim_multi_push)
    
    if sim_multi_push_debiased is not None:
        ax11 = fig.add_subplot(gs[row_i]); row_i += 1
        plot_curve(ax11, f"Debiased Multi-Ref Cos-Push", sim_multi_push_debiased, xlabel=True)
    
    # else:
    #     # If multi-row not present, give xlabel to the last plotted axis (ax7 or ax8)
    #     (ax11 if sim_multi_push_debiased is not None else (ax10 if sim_multi_push is not None else ax9 )).set_xlabel("Frame centers (pixel aligned)")
    ...
    if args.print_topk:
        ...
        if sim_multi_taf_debiased is not None:
            sim_multi_taf_debiased_col = [c for c in df.columns if c.startswith("sim_multi_taf_debiased_beta_")][0]
            print_topk(df[sim_multi_taf_debiased_col], f"Multi-Ref TAF debiased Retrieval (λ={push_lambda:.2f}, K={multi_ref_count})", args.topk)
        if sim_multi_push_debiased is not None:
            sim_multi_push_debiased_col = [c for c in df.columns if c.startswith("sim_multi_push_debiased_lambda_")][0]
            print_topk(df[sim_multi_push_debiased_col], f"Multi-Ref Cos-Push debiased Retrieval (λ={push_lambda:.2f}, K={multi_ref_count})", args.topk)
